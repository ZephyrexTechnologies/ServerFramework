...
# TODO Add a static init hooks list that extensions can use to create properies etc on the classes.
...
class AbstractBLLManager:
    ...
    def get_field_types(self):
        ...
        for field_name, field_info in self.Model.__annotations__.items():
            ...
            elif actual_type in (datetime.date, datetime.datetime):
        ...
        for field_name, value in search_params.items():
            ...
            # Handle date field operations
                ...
                if "on" in value and value["on"] is not None:
                    ...
                    if isinstance(field.type, (datetime.date, datetime.datetime)):
                        on_date = value["on"]
                        conditions.append(
                            and_(
                                field >= on_date.replace(hour=0, minute=0, second=0),
                                field
                                < (on_date + datetime.timedelta(days=1)).replace(
    ...
    def search(
        ...
        **search_params,
        ...
        # TODO Figure out why this breaks search. Optionals are being flagged required.
        # if search_params:
        #     self.Model.Search(**search_params)
        # else:
        # If no search parameters, fall back to list with converted options
        return self.list(
            include=include,
            fields=fields,
            filters=filters,
        ...
        # Pass the converted SQLAlchemy constructs to the Mixins.list method
        return self.DBClass.list(
            requester_id=self.requester.id,
            db=self.db,
            return_type="dto",
